...
namespace AssetStoreTools.Uploader.UI.Views
{
    internal class LoginView : VisualElement
    {
        ...
        {
            _loginButton = new Button(OnLoginClicked) { name = "LoginButton" };
            _loginButton.AddToClassList("login-button");
            _loginButton.SetEnabled(true);
            Add(_loginButton);
        }
        private void CreateLoadingSpinner()
        {
            _loadingSpinner = new Label { name = "LoadingSpinner" };
        ...
        private async void OnLoginClicked()
        {
            try
            {
                ...
                var username = _emailField.value;
                var password = _passwordField.value;
                if (string.IsNullOrEmpty(username) || string.IsNullOrEmpty(password))
                {
                    DisplayError("Lütfen kullanıcı adı ve şifrenizi girin");
                    return;
                }
                // Kimlik doğrulama işlemi
                var authResult = await _authenticationService.AuthenticateWithCredentials(username, password);
                if (!authResult.Success)
                {
                    DisplayError($"Giriş başarısız: {authResult.Exception.Message}");
                    return;
                }
                // Başarılı giriş sonrası paket listesini yükle
                OnLoginSuccess(authResult.User);
            }
            catch (Exception ex)
            {
                Debug.LogError($"Giriş hatası: {ex.Message}");
                DisplayError("Sunucuya bağlanılamıyor. Lütfen internet bağlantınızı kontrol edin");
            }
            finally
            {
        ...
        {
            _loadingSpinner.style.display = show ? DisplayStyle.Flex : DisplayStyle.None;
        }
    }
}